...
def _flash_attn_or_sdpa(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, causal: bool = True) -> torch.Tensor:
    """Dispatch attention: FA3 > FA2 > PyTorch SDPA. All expect (B, T, H, D)."""
    if _ATTN_BACKEND == "fa3":
        return _flash_attn_3_func(q, k, v, causal=causal)
    if _ATTN_BACKEND == "fa2":
        return _flash_attn_2_func(q, k, v, causal=causal)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        y = _flash_attn_or_sdpa(q, k, v, causal=True)
        if self.use_xsa:
...
def eval_val_sliding(
    ...
    compiled_logits = torch.compile(base_model.forward_logits, dynamic=False, fullgraph=True) if _use_compile else base_model.forward_logits
...
def main() -> None:
    ...
    if _ATTN_BACKEND in ("fa3", "fa2"):
        # FA handles attention directly; disable SDP backends
        enable_cudnn_sdp(False)
        enable_flash_sdp(True)
        enable_mem_efficient_sdp(False)
        enable_math_sdp(False)
    else:
        # SDPA fallback: let PyTorch pick the best available backend
    ...
    while True:
        ...
        if should_validate:
            ...
            if monitor:
                monitor.check_val(step, val_loss, val_bpb, training_time_ms, is_last_step=last_step)
    ...
    sw_val_loss, sw_val_bpb = None, None
    ...
    if monitor:
        monitor.emit_final(
            ...
            bytes_total=(quant_file_bytes + code_bytes) if master_process else 0,
            bytes_model=quant_file_bytes if master_process else 0,
            bytes_code=code_bytes if master_process else 0,
