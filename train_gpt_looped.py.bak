...
class Muon(torch.optim.Optimizer):
    def __init__(self, params, lr: float, momentum: float, backend_steps: int, nesterov: bool = True, beta2: float = 0.0):
        ...
        self._updates_flat = None  # lazily allocated on first step
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            if self._updates_flat is None or self._updates_flat.numel() != total_params:
...
def get_logits(base_model: nn.Module, input_ids: Tensor) -> Tensor:
    ...
    if base_model.tie_embeddings:
        ...
        logits_proj = base_model.lm_head(x)
    return base_model.logit_softcap * torch.tanh(logits_proj / base_model.logit_softcap)
...
EMBED_PASSTHROUGH_PATTERNS = ("tok_emb.weight", "embedding.weight", "wte.weight")
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        if EMBED_FP16 and any(pat in name for pat in EMBED_PASSTHROUGH_PATTERNS):
            passthrough_orig_dtypes[name] = str(t.dtype).removeprefix("torch.")
...
def main() -> None:
    ...
    if base_model.lm_head is not None:
        adam_param_groups.append({"params": [base_model.lm_head.weight], "lr": args.head_lr, "base_lr": args.head_lr})
    optimizer_adam = torch.optim.Adam(adam_param_groups, betas=(args.beta1, args.beta2), eps=args.adam_eps, fused=True)
    optimizer_muon = Muon(
        matrix_params,
        lr=args.matrix_lr,
